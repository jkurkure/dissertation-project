...
function ResContext() {
	...
	// forced direction
	this.dir = null; // for RESlite can be null, "lr", "rl"
}
...
function(str, size, xScale, font, rotate, mirror) {
	...
	var ctx = measCanvas.getContext("2d", { willReadFrequently: true });
	ctx.font = sizedFont;
	ctx.fillStyle = "black";
	...
	for (var i = 0; i < maxRepeats; i++) {
		...
		measCanvas.width = w;
		measCanvas.height = h;
		ctx = measCanvas.getContext("2d", { willReadFrequently: true });
		ctx.font = sizedFont;
		ctx.fillStyle = "black";
		ctx.textBaseline = "alphabetic";
...
ResFragment.prototype.nSymbols =
function() {
	return this.hiero === null ? 0 : this.hiero.nSymbols();
}
...
ResHieroglyphic.prototype.nSymbols =
function() {
	var n = 0
	for (var i = 0; i < this.groups.length; i++) 
		n += this.groups[i].nSymbols();
	return n;
};
...
ResVertgroup.prototype.nSymbols =
function() {
	var n = 0;
	for (var i = 0; i < this.groups.length; i++)
		n += this.groups[i].nSymbols();
...
ResVertsubgroup.prototype.nSymbols =
function() {
	return this.group.nSymbols();
};
...
ResHorgroup.prototype.nSymbols =
function() {
	var n = 0;
	for (var i = 0; i < this.groups.length; i++)
		n += this.groups[i].nSymbols();
...
ResHorsubgroup.prototype.nSymbols =
function() {
	return this.group.nSymbols();
};
...
ResNamedglyph.prototype.nSymbols =
function () {
	return 1;
};
...
ResEmptyglyph.prototype.nSymbols =
function () {
	return 0;
};
...
ResStack.prototype.nSymbols =
function () {
	return this.group1.nSymbols() + this.group2.nSymbols();
};
...
function(canvas, size) {
	...
	while (true) {
		...
		env.ctx = canvas.getContext("2d", { willReadFrequently: true });
...
function(i) {
	...
	envBefore.ctx = canvasBefore.getContext("2d", { willReadFrequently: true });
	...
	envAfter.ctx = canvasAfter.getContext("2d", { willReadFrequently: true });
...
function() {
	...
	env1.ctx = canvas1.getContext("2d", { willReadFrequently: true });
	env1.mirror = false;
	...
	env2.ctx = canvas2.getContext("2d", { willReadFrequently: true });
	env2.mirror = false;
...
function(font, sign1, sign2, ratio) {
	...
	var ctx = measureCanvas.getContext("2d", { willReadFrequently: true });
	ctx.font = font;
...
function ResTierPart(str, size) {
	this.str = str;
	this.frag = res_syntax.parse(str);
	this.context = new ResContext();
	this.context.emSizePx = size;
	this.frag.format(this.context);
	try {
		this.nSyms = this.frag.nSymbols();
	} catch(err) {
		console.log(this.frag);
	}
	this.breakables = Array(this.nSyms).fill(false);
	this.widths = Array(this.nSyms).fill(0);
	var nSum = 0;
	var wSum = 0;
	if (this.frag.hiero !== null) {
		const groups = this.frag.hiero.groups;
		const ops = this.frag.hiero.ops;
		for (let i = 0; i < groups.length; i++) {
			const group = groups[i];
			const n = group.nSymbols();
			var width = 0;
			if (i > 0)
				width += this.frag.hiero.ops[i-1].sizePx();
			width += group.widthPx();
			if (nSum+n < this.nSyms)
				this.breakables[nSum+n] = true;
			for (let m = nSum+1; m < Math.max(nSum+n, this.nSyms); m++)
				this.widths[m] = wSum + width;
			nSum += n;
			wSum += width;
		}
	}
...
ResTierPart.prototype.widthPx =
function () {
	return this.frag.widthPx();
};

ResTierPart.prototype.nSymbols =
function () {
    return this.nSyms;
};

ResTierPart.prototype.breakable =
function (i) {
    return i >= this.nSyms || this.breakables[i];
};

ResTierPart.prototype.widthPxTo = 
function (n) {
    return n >= this.nSyms ? this.widthPx() : this.widths[n];
};

ResTierPart.prototype.split =
function (nSymbols) {
	if (this.frag.hiero === null)
		return { prefix: '', suffix: '' };
	const hiero = this.frag.hiero;
	const groups = hiero.groups;
	const ops = hiero.ops;
	const switches = hiero.switches;
	const globalss = hiero.globalss;
	var nGroups = 0;
	var nSum = 0;
	for (let i = 0; i < groups.length; i++) {
		const n = groups[i].nSymbols();
		if (nSymbols <= nSum + n) {
			nGroups = i+1;
			break;
		}
	...
	// var hiero1 = new ResHieroglyphic({ g: groups.slice(0, nGroups) });
	var prefix = '';
	var suffice = '';
	var globalsMid = null;
	if (nGroups > 0) {
		const groups1 = groups.slice(0, nGroups);
		const ops1 = ops.slice(0, nGroups-1);
		const switches1 = switches.slice(0, nGroups-1);
		const hiero1 = new ResHieroglyphic({ groups: groups1, ops: ops1, switches: switches1 });
		const globalsStart = globalss[0];
		const sw1 = new ResSwitch(globalsStart);
		const frag1 = new ResFragment({ direction: this.frag.direction, size: this.frag.size, 
			switchs: sw1, hiero: hiero1 });
		prefix = frag1.toString();
		globalsMid = globalss[nGroups];
	}

	if (nGroups < this.frag.hiero.groups.length) {
		const groups2 = groups.slice(nGroups);
		const ops2 = ops.slice(nGroups);
		const switches2 = switches.slice(nGroups);
		const hiero2 = new ResHieroglyphic({ groups: groups2, ops: ops2, switches: switches2 });
		const sw2 = globalsMid ? new ResSwitch(globalsMid) : new ResSwitch(null);
		const frag2 = new ResFragment({ direction: this.frag.direction, size: this.frag.size, 
			switchs: sw2, hiero: hiero2 });
		suffix = frag2.toString();
	}
	return { prefix, suffix };
};
